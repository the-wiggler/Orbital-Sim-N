...
void body_calculateSOI(body_properties_t* global_bodies) {
    if (global_bodies->count < 2) { return; }  // need at least 2 bodies

    body_t* central_body = &global_bodies->bodies[0];
    const double central_mass = central_body->mass;

    // first body has no SOI
    central_body->SOI_radius = 0.0;

    for (int i = 1; i < global_bodies->count; i++) {
        body_t* body = &global_bodies->bodies[i];

        // calculate distance from central body
        const vec3 delta = vec3_sub(body->pos, central_body->pos);
        const double accel = vec3_mag(delta);

        // SOI = a * (m/M)^(2/5)
        const double mass_ratio = body->mass / central_mass;

        body->SOI_radius = accel * pow(mass_ratio, 0.4);
    }
}
...
void body_findClosestPlanet(body_t* body, const body_properties_t* global_bodies) {
    ...
    for (int i = 0; i < global_bodies->count; i++) {
        ...
        const vec3 delta_pos = vec3_sub(global_bodies->bodies[i].pos, body->pos);
        const double r_squared = vec3_mag_sq(delta_pos);
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
void body_addOrbitalBody(body_properties_t* global_bodies, const char* name, const double mass, const double radius, const vec3 pos, const vec3 vel) {
    // check bounds
    if (global_bodies->count >= MAX_PLANETS) {
        char err_txt[MAX_ERR_SIZE];
    ...
    const int idx = global_bodies->count;
    body_t* body = &global_bodies->bodies[idx];
